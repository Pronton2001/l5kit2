...
def visualize3(scene_index: int, frames: List[FrameVisualization], button) -> LayoutDOM:
    """Visualise a scene using Bokeh.
    ...
    button_callback = CustomJS(
        args=dict(figure=f, sources=out[0], frames=out, btn = button),
        code = js_string
    )
    ...
    # callback = CustomJS(args=dict(figure=f, sources=out[0], frames=out), code=js_string)
    # out[0]['ego'].js_on_change('foo', callback)
    # f.js_on_event('layout_rendered', callback)
